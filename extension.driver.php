<?php
	...
	define_safe(MMK_NAME, 'Field: Multilingual Meta Keys');
	define_safe(MMK_GROUP, 'multilingual_metakeys');
	...
	class Extension_Multilingual_Metakeys extends Extension
	{
		...
		public function dAddCustomPreferenceFieldsets($context){
			...
			$label->appendChild(Widget::Input('settings['.MMK_GROUP.'][consolidate]', 'yes', 'checkbox', array('checked' => 'checked')));
		...
		public function dFLSavePreferences($context){
			...
			if( is_array($fields) && !empty($fields) ){
				$consolidate = $context['context']['settings'][MMK_GROUP]['consolidate'];
		...
		public static function appendAssets(){
			if( self::$assets_loaded === false
				...
				$page->addStylesheetToHead(URL.'/extensions/'.MMK_GROUP.'/assets/'.MMK_GROUP.'.publish.css', 'screen', null, false);
			}
		}
	}
